...
master_promotion_order(resource_t * rsc, pe_working_set_t * data_set)
{
    ...
    for (; gIter != NULL; gIter = gIter->next) {
        ...
        if (constraint->role_lh == RSC_ROLE_MASTER) {
            crm_debug_2("RHS: %s with %s: %d", constraint->rsc_lh->id, constraint->rsc_rh->id,
                        constraint->score);
            rsc->allowed_nodes =
    ...
    for (; gIter != NULL; gIter = gIter->next) {
        ...
        if (constraint->role_rh == RSC_ROLE_MASTER) {
            crm_debug_2("LHS: %s with %s: %d", constraint->rsc_lh->id, constraint->rsc_rh->id,
                        constraint->score);
            rsc->allowed_nodes =
...
master_score(resource_t * rsc, node_t * node, int not_set_value)
{
    ...
    if (node != NULL) {
        ...
        if (match == NULL) {
            crm_debug_2("%s is not active on %s - ignoring", rsc->id, node->details->uname);
            return score;
        }
...
master_color(resource_t * rsc, node_t * prefer, pe_working_set_t * data_set)
{
    ...
    for (; gIter != NULL; gIter = gIter->next) {
        ...
        crm_debug_2("Assigning priority for %s: %s", child_rsc->id,
...
master_create_actions(resource_t * rsc, pe_working_set_t * data_set)
{
    ...
    for (; gIter != NULL; gIter = gIter->next) {
        ...
        crm_debug_2("Created actions for %s: %d %d", child_rsc->id, child_promoting,
                    child_demoting);
    }
