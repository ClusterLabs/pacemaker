...
pe__filter_rsc_list(GList *rscs, GList *filter)
{
    ...
    for (GList *gIter = rscs; gIter; gIter = gIter->next) {
         ...
         */
        if (pcmk__str_in_list(filter, rsc_printable_id(rsc), pcmk__str_star_matches) ||
            (rsc->parent && pcmk__str_in_list(filter, rsc_printable_id(rsc->parent), pcmk__str_star_matches))) {
