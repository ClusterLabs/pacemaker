...
unpack_nodes(xmlNode * xml_nodes, pe_working_set_t *data_set)
{
    ...
    for(xml_obj = xml_nodes; xml_obj != NULL; xml_obj = xml_obj->next) {
...
unpack_domains(xmlNode *xml_domains, pe_working_set_t *data_set)
{
    ...
    for(xml_domain = xml_domains; xml_domain != NULL; xml_domain = xml_domain->next) {
	if(crm_str_eq((const char *)xml_domain->name, XML_CIB_TAG_DOMAIN, TRUE)) {
	    domain = NULL;
	    id = crm_element_value(xml_domain, XML_ATTR_ID);

	    for(xml_node = xml_domain; xml_node != NULL; xml_node = xml_node->next) {
...
{
    xmlNode *xml_obj = NULL;
    for(xml_obj = xml_resources; xml_obj != NULL; xml_obj = xml_obj->next) {
...
unpack_lrm_resources(node_t *node, xmlNode * lrm_rsc_list, pe_working_set_t *data_set)
{
    ...
    for(rsc_entry = lrm_rsc_list; rsc_entry != NULL; rsc_entry = rsc_entry->next) {
    ...
    const char *rsc, const char *node, gboolean active_filter, pe_working_set_t *data_set) 
{
	...
	if(crm_str_eq((const char *)node_state->name, XML_CIB_TAG_STATE, TRUE)) {
		...
		for(lrm_rsc = tmp; lrm_rsc != NULL; lrm_rsc = lrm_rsc->next) {
