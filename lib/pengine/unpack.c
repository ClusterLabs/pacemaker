...
unpack_config(xmlNode *config, pcmk_scheduler_t *scheduler)
{
    ...
    value = pcmk__cluster_option(config_hash, PCMK_OPT_FENCE_REMOTE_WITHOUT_QUORUM);
    if (value != NULL && !crm_is_true(value)) {
        crm_warn("Experimental fencing option enabled: remote nodes may not "
                 "be fenced in inquorate partition");
