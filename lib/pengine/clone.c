...
pe__clone_xml(pcmk__output_t *out, va_list args)
{
    ...
    print_everything = pcmk__str_in_list(only_rsc, rsc_printable_id(rsc), pcmk__str_star_matches) ||
...
pe__clone_is_filtered(pe_resource_t *rsc, GList *only_rsc, gboolean check_parent)
{
    ...
    if (pcmk__str_in_list(only_rsc, rsc_printable_id(rsc), pcmk__str_star_matches)) {
        ...
        get_clone_variant_data(clone_data, rsc);
        passes = pcmk__str_in_list(only_rsc, ID(clone_data->xml_obj_child), pcmk__str_star_matches);
