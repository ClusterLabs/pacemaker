...
ban_list(pcmk__output_t *out, va_list args) {
    ...
    for (gIter = data_set->placement_constraints; gIter != NULL; gIter = gIter->next) {
        ...
        if (!pcmk__str_in_list(only_rsc, rsc_printable_id(location->rsc_lh), pcmk__str_star_matches) &&
            !pcmk__str_in_list(only_rsc, rsc_printable_id(uber_parent(location->rsc_lh)), pcmk__str_star_matches)) {
            continue;
        }
...
failed_action_list(pcmk__output_t *out, va_list args) {
    ...
    for (xml_op = pcmk__xml_first_child(data_set->failed); xml_op != NULL;
        ...
        if (!pcmk__str_in_list(only_rsc, rsc, pcmk__str_star_matches)) {
            free(rsc);
            continue;
        }
...
node_attribute_list(pcmk__output_t *out, va_list args) {
    ...
    for (GList *gIter = data_set->nodes; gIter != NULL; gIter = gIter->next) {
        ...
        if (!pcmk__str_in_list(only_node, node->details->uname, pcmk__str_star_matches|pcmk__str_casei)) {
    ...
    for (rsc_entry = first_named_child(lrm_rsc, XML_LRM_TAG_RESOURCE);
        ...
        if (uber_parent(rsc)->variant == pe_group) {
            if (!pcmk__str_in_list(only_rsc, rsc_printable_id(rsc), pcmk__str_star_matches) &&
                !pcmk__str_in_list(only_rsc, rsc_printable_id(uber_parent(rsc)), pcmk__str_star_matches)) {
                continue;
            }
        } else {
