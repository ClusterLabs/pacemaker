                      ...
                      const char *name, const char *value)
{
    ...
    attrs = pcmk__xe_match_name(cib_node, PCMK__XE_TRANSIENT_ATTRIBUTES);
    if (attrs == NULL) {
        attrs = pcmk__xe_create(cib_node, PCMK__XE_TRANSIENT_ATTRIBUTES);
        crm_xml_add(attrs, PCMK_XA_ID, node_uuid);
    }

    instance_attrs = pcmk__xe_match_name(attrs, PCMK_XE_INSTANCE_ATTRIBUTES);
                              ...
                              const char *rprovider)
{
    ...
    lrm = pcmk__xe_match_name(cib_node, PCMK__XE_LRM);
    if (lrm == NULL) {
        const char *node_uuid = pcmk__xe_id(cib_node);
    ...
    container = pcmk__xe_match_name(lrm, PCMK__XE_LRM_RESOURCES);
    if (container == NULL) {
