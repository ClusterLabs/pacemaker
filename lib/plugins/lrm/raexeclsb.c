...
static int fork_and_execra(const char * ra_name, const char * op, 
			RA_ARGV ra_argv, GHashTable * env_params, 
			gboolean need_metadata);
	...
	GHashTable * env_params, gboolean need_metadata, int * call_key)
{
	...
	*call_key = fork_and_execra(ra_name, op, params_argv, env_params, 
					need_metadata);
...
static int 
fork_and_execra(const char * ra_name, const char * op, RA_ARGV ra_argv,
		GHashTable * env_params, gboolean need_metadata)
{
	...
	/* For lsb init scripts, no corresponding definite specification
	 * Not set calling parameterscl. But for lsb none-init scripts,
	 * should need
	 */
		raexec_setenv(env_params);
		if ( execv(ra_name, ra_argv) < 0 ) {
