...
#define ACL_RENDER_CFG "acls-render-cfg"
#define ACL_NS_CFG "http://clusterlabs.org/ns/pacemaker/" ACL_RENDER_CFG
#define ACL_NS_PREFIX "http://clusterlabs.org/ns/pacemaker/acl-2-"
#define ACL_REDIRNS_PREFIX "http://clusterlabs.org/nsredir/pacemaker/cfg/"
#define ACL_NS_Q_WRITABLE (const xmlChar *) "pcmk-acl-writable"
#define ACL_NS_Q_READABLE (const xmlChar *) "pcmk-acl-readable"
#define ACL_NS_Q_DENIED (const xmlChar *) "pcmk-acl-denied"
                              ...
                              xmlDoc **acl_evaled_doc)
{
    ...
    char comment_buf[256] = "ACLs as evaluated for user ";
                                           ...
                                           xsltLoadType type)
{
    ...
    if (override_redir_target != NULL
            && crm_starts_with((const char *) URI, ACL_REDIRNS_PREFIX)) {
        URI_use = crm_strdup_printf("file://%s/%s", override_redir_target,
                                    URI + sizeof(ACL_REDIRNS_PREFIX) - 1);
    }
                        ...
                        xmlChar **doc_txt_ptr, int *doc_txt_len)
{
    ...
    static const char *params_ns_simple[] = {
        "aclsrendercfg:c-writable",           "writable",
        "aclsrendercfg:c-readable",           "readable",
        "aclsrendercfg:c-denied",             "denied",
        "aclsrendercfg:c-reset",              "",
        "aclsrender:extra-spacing",           "no",
        "aclsrender:self-reproducing-prefix", "pcmk-acl-",
        NULL
    }, *params_useansi[] = {
        /* start with hard-coded defaults, then adapt per the template ones */
        "aclsrendercfg:c-writable",           "\x1b[32m",
        "aclsrendercfg:c-readable",           "\x1b[34m",
        "aclsrendercfg:c-denied",             "\x1b[31m",
        "aclsrendercfg:c-reset",              "\x1b[0m",
        "aclsrender:extra-spacing",           "no",
        "aclsrender:self-reproducing-prefix", "",
        NULL
    }, *params_noansi[] = {
        "aclsrendercfg:c-writable",           "vvv---[ WRITABLE ]---vvv",
        "aclsrendercfg:c-readable",           "vvv---[ READABLE ]---vvv",
        "aclsrendercfg:c-denied",             "vvv---[ ~DENIED~ ]---vvv",
        "aclsrendercfg:c-reset",              "",
        "aclsrender:extra-spacing",           "yes",
        "aclsrender:self-reproducing-prefix", "",
        NULL
    };
    const char **params;
    ...
    if (how == pcmk__acl_render_color) {
        divert_xslt_doc_loader(
            prioritized_checked_config_dir(ACL_RENDER_CFG ".xsl",
                                           getenv("PCMK_config_directory")),
            true
        );
    }
    ...
    res = xmlReadDoc(annotated_dump, "on-the-fly-ACLs-render", NULL,
                     XML_PARSE_NONET);
    CRM_ASSERT(res != NULL);
