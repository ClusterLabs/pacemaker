...
crm_time_get_sec(int sec, uint32_t *h, uint32_t *m, uint32_t *s)
{
    ...
    pcmk__trace("%d == %.2" PRIu32 ":%.2" PRIu32 ":%.2" PRIu32, sec, hours,
                minutes, seconds);
                      ...
                      char *result)
{
    ...
    if (pcmk__is_set(flags, crm_time_log_duration)) {
        crm_duration_as_string(dt, usec, pcmk__is_set(flags, crm_time_usecs),
                               result);
        return;
    }
    ...
    *result = 0;
    ...
    if ((hour == 24) && (minute == 0) && (second == 0)) {
        ...
        pcmk__err("%s is not a valid ISO 8601 time specification "
                  "because %" PRIu32 " is not a valid hour",
                  time_str, hour);
        errno = EINVAL;
        return FALSE;
    }
    if (minute >= 60) {
        pcmk__err("%s is not a valid ISO 8601 time specification "
                  "because %" PRIu32 " is not a valid minute",
                  time_str, minute);
        errno = EINVAL;
        return FALSE;
    }
    if (second >= 60) {
        pcmk__err("%s is not a valid ISO 8601 time specification "
                  "because %" PRIu32 " is not a valid second",
                  time_str, second);
        errno = EINVAL;
        return FALSE;
    }
...
parse_date(const char *date_str)
{
    ...
    if (rc == 3) {
        if ((month < 1U) || (month > 12U)) {
            ...
            pcmk__trace("Parsed Gregorian date '%.4" PRIu32 "-%.3d' from date "
                        "string '%s'",
...
#define do_cmp_field(l, r, field)                                   \
    if (rc == 0) {                                                  \
        if (l->field > r->field) {                                  \
            pcmk__trace("%s: %d > %d", #field, l->field, r->field); \
            rc = 1;                                                 \
