...
ascii(void **state)
{
    for (int c = 0x00; c <= 0x7F; c++) {
        if (g_ascii_isalpha(c) || c == ':' || c == '_') {
            assert_name_start_char(c);
        } else {
            assert_not_name_start_char(c);
        }
    }
}
...
{
    assert_not_name_start_char(0xD7);
}
...
{
    assert_not_name_start_char(0xF7);
}
...
{
    assert_not_name_start_char(0x37E);
}
...
unicode_0xD800_to_0xDFFF(void **state)
{
    ...
    for (int c = 0xD800; c <= 0xDFFF; c++) {
        ...
        assert_false(pcmk__xml_is_name_start_char(utf8_buf, NULL));
    }
}
