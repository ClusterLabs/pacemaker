...
snprintf_error(void **state)
{
    ...
    // setenv() not called
    expect_any(pcmk__test_snprintf, str);
    expect_value(pcmk__test_snprintf, size, NAME_MAX);
    expect_string(pcmk__test_snprintf, format, "%s%s");
    will_return(pcmk__test_snprintf, EOVERFLOW);
