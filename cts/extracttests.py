...
class ExtractTests:
    ...
    def __init__(self, filename, testlist):
        ...
        self.regex=re.compile(" CTS: Running test .*(\[|\s)([0-9]+)")
        self.CTSregex=re.compile(" CTS: ")
        self.DEBUGregex=re.compile(" CTS: (BadNews|debug):")
    ...
    def __call__(self):
        ...
        while 1:
            ...
            if regexmatchobj:
                curtest= int(regexmatchobj.groups()[1])

            if curtest == 0:
                # Print the summary
                if self.CTSregex.search(line):
                   if not self.DEBUGregex.search(line):
