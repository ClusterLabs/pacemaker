...
clear_constraints(pcmk__output_t *out, xmlNodePtr *cib_xml_copy)
{
    ...
    if (options.clear_expired) {
        ...
        dest = pcmk__find_node_in_list(scheduler->nodes, options.host_uname);
        if (dest == NULL) {
...
refresh(pcmk__output_t *out)
{
    ...
    if (options.host_uname) {
        pcmk_node_t *node = pcmk__find_node_in_list(scheduler->nodes, options.host_uname);
...
main(int argc, char **argv)
{
    ...
    if ((options.host_uname != NULL) && (scheduler != NULL)) {
        node = pcmk__find_node_in_list(scheduler->nodes, options.host_uname);
