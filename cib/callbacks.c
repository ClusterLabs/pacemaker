...
cib_ipc_connection_destroy(gpointer user_data)
{
    ...
    if (cib_client->source != NULL) {
        ...
        crm_trace("Deleting %s (%p) from mainloop", cib_client->name, cib_client->source);
...
cib_client_connect(IPC_Channel * channel, gpointer user_data)
{
    ...
    new_client->source = G_main_add_IPC_Channel(G_PRIORITY_DEFAULT, channel, FALSE, callback,
                           ...
                           gboolean force_synchronous, gboolean privileged)
{
    ...
    cib_process_request(op_request, force_synchronous, privileged, FALSE, cib_client);
                    ...
                    gboolean force_synchronous, gboolean privileged)
{
     ...
     */
    while (lpc < 5 && IPC_ISRCONN(channel)
        ...
        cib_common_callback_worker(op_request, cib_client, force_synchronous, privileged);
                    ...
                    gboolean * process, gboolean * needs_forward)
{
    ...
    if (host == NULL && (call_options & cib_scope_local)) {
        crm_debug_2("Processing locally scoped %s op from %s", op, cib_client->name);
        *local_notify = TRUE;

    } else if (host == NULL && cib_is_master) {
        crm_debug_2("Processing master %s op locally from %s", op, cib_client->name);
        *local_notify = TRUE;

    } else if (safe_str_eq(host, cib_our_uname)) {
        crm_debug_2("Processing locally addressed %s op from %s", op, cib_client->name);
        *local_notify = TRUE;
                    ...
                    gboolean from_peer, cib_client_t * cib_client)
{
    ...
    crm_debug_4("%s Processing msg %s", cib_our_uname, crm_element_value(request, F_SEQ));
    ...
    if (needs_reply == FALSE || stand_alone) {
        ...
        if (is_update == FALSE || result_diff == NULL) {
            ...
            crm_debug_3("Request not broadcast: call failed: %s", cib_error2string(rc));
        } else {
...
send_via_callback_channel(xmlNode * msg, const char *token)
{
    ...
    if (rc == cib_ok) {
        crm_debug_3("Delivering reply to client %s (%s)", token, hash_client->channel_name);
        if (crm_str_eq(hash_client->channel_name, "remote", FALSE)) {
            ...
            crm_warn("Delivery of reply to client %s/%s failed", hash_client->name, token);
            rc = cib_reply_failed;
        }
    }
...
cib_process_disconnect(IPC_Channel * channel, cib_client_t * cib_client)
{
    ...
    if (channel == NULL) {
        ...
        crm_debug_2("Cleaning up after client disconnect: %s/%s/%s",
                    crm_str(cib_client->name), cib_client->channel_name, cib_client->id);
...
cib_ccm_msg_callback(oc_ed_t event, void *cookie, size_t size, const void *data)
{
    ...
    switch (event) {
        ...
        case OC_EV_MS_NOT_PRIMARY:
            crm_debug_2("Ignoring transitional CCM event: %s", ccm_event_name(event));
            break;
