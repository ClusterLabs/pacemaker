...
int
init_status(const char *pid_file, const char *daemon_name)
{
    gboolean	anypidfile;
		 ...
		 GDestroyNotify cleanup_method)
{
    ...
    if (hb_cluster->llc_ops->signon(hb_cluster, daemon_name)!= HA_OK) {
...
/*     } */
    ...
    if (hb_cluster->llc_ops->set_msg_callback(hb_cluster, "CRM", message_callback, hb_cluster)
	!=HA_OK){
