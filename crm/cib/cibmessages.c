...
xmlNodePtr createCibRequest(gboolean isLocal,
							const char *operation, const char *section,
...
processCibRequest(xmlNodePtr command)
{
    ...
    if (command == NULL)
    {
		cl_log(LOG_INFO, "The (%s) received an empty message",
			   CRM_SYSTEM_CIB);
		FNRET(NULL);
    }
    ...
    xmlNodePtr cib_answer = create_xml_node(NULL, XML_RESP_TAG_CIB);
    ...
    else if (strcmp("query", op) == 0)
    {
		...
		/* force a pick-up of the relevant section before returning */
		verbose = "true"; 
    ...
    else if (strcmp("replace", op) == 0)
    {
		...
		{
			xmlNodePtr new_cib = find_xml_node(command, XML_TAG_CIB);
		...
		{
			xmlNodePtr new_section = find_xml_node(command, XML_TAG_CIB);
			...
			if (new_section != NULL)
			{
				...
				xmlNodePtr old_section = find_xml_node(tmpCib, section);
		...
		if (strcmp("all", section) == 0)
		{
				...
				|| cib_update_operation == CIB_OP_MODIFY)
			{
				updateList(tmpCib, command, failed,
						   cib_update_operation, XML_CIB_TAG_NODES);
				updateList(tmpCib, command, failed,
						   cib_update_operation, XML_CIB_TAG_RESOURCES);
				updateList(tmpCib, command, failed,
						   cib_update_operation, XML_CIB_TAG_CONSTRAINTS);
				updateList(tmpCib, command, failed,
						   cib_update_operation, XML_CIB_TAG_STATUS);
			}
			else // delete
			{
				updateList(tmpCib, command, failed,
		   ...
		   int operation, const char *section)
{
    ...
    xmlNodePtr xml_section =
		find_xml_node_nested(update_command, node_path, 3);
    ...
    while(child != NULL)
    {
			...
			&& strcmp(XML_CIB_TAG_NODE, child->name) == 0)
		{
			if (operation == CIB_OP_ADD)
				conditional_add_failure(failed, child, operation,
				 ...
				 && strcmp(XML_CIB_TAG_CONSTRAINT, child->name) == 0)
		{
			if (operation == CIB_OP_ADD)
				...
				conditional_add_failure(failed, child, operation,
										delConstraint(local_cib, ID(child)));
		}
		else if (strcmp(type_check, child->name) == 0
						...
						xmlNodePtr failed)
{
    ...
    if (data != NULL) {
		if (data->name == NULL) {
			cl_log(LOG_ERR, "Data being added had no name, discarding.");
