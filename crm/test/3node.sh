...
do_cmd ./testutils.pl -l ${logfile} --search -a -m 1500 -s "${test_node_3} ccm(.*) Hostname: ${test_node_3}" -s "${test_node_3} heartbeat(.*): info: Starting (.*)lrmd" -e "${test_node_3} heartbeat(.*) Client (.*) respawning too fast"
...
#do_cmd wait_for_state S_INTEGRATION 30 $test_node_1 
#cts_assert "S_INTEGRATION not reached on $test_node_1 (new node)!"
do_cmd echo "wait for CRMd to start on $test_node_3"
do_cmd ./testutils.pl -l ${logfile} --search -a -m 1500  -s "${test_node_3} crmd(.*)FSA Hostname: ${test_node_3}"
