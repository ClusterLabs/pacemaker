...
do_cmd ./testutils.pl --search -a -m 1500 -s "${test_node_3} ccm(.*) Hostname: ${test_node_3}" -s "${test_node_3} heartbeat(.*): info: Starting (.*)lrmd" -e "${test_node_3} heartbeat(.*) Client (.*) respawning too fast"
...
do_cmd ./testutils.pl --search -a -m 1500  -s "${test_node_3} crmd(.*)FSA Hostname: ${test_node_3}"
