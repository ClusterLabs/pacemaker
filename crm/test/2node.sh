...
do_cmd ./testutils.pl -l ${logfile} --search -a -m 1500 -s "${test_node_2} ccm(.*) Hostname: ${test_node_2}" -s "${test_node_2} heartbeat(.*): info: Starting (.*)lrmd" -e "${test_node_2} heartbeat(.*) Client (.*) respawning too fast"
...
do_cmd ./testutils.pl -l ${logfile} --search -a -m 1500  -s "${test_node_2} crmd(.*)FSA Hostname: ${test_node_2}" -s "${test_node_2} crmd(.*) Performing op start(.*) on rsc2"
cts_assert "CRMd startup on ${test_node_2} failed."

#do_cmd echo "wait for transition to complete"
#do_cmd ./testutils.pl -l ${logfile} --search $logfile '"${test_node_2} crmd(.*)erforming op start(.*) on rsc2"' 1 1500
...
do_cmd ./testutils.pl -l ${logfile} --search -a -m 1500 -s "${test_node_2} crmd(.*)State transition \"S_PENDING\" -> \"S_STOPPING\""
