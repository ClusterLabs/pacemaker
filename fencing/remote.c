...
call_remote_stonith(remote_fencing_op_t * op, st_query_result_t * peer)
{
    ...
    if (peer) {
        ...
        /* why was there a additional check (device == NULL || safe_str_eq(device, STONITH_WATCHDOG_ID))
         * that prevented fall through to watchdog !!!! ????
         */
        if(stonith_watchdog_timeout_ms) {
            if ((stonith_watchdog_targets == NULL) ||
                string_in_list(stonith_watchdog_targets, op->target)) {
                crm_notice("Waiting %ds for %s to self-fence (%s) for %s.%.8s (%p)",
                           stonith_watchdog_timeout_ms/1000, op->target,
                           op->action, op->client_name, op->id, device);
