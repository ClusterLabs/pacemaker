...
acls_render_prep() {
	test -s pacemaker-acls-3.0.rng || {
	    (
	    export OUTPUTDIR=test-acls-render \
	        SKIPSCHEMAS=".*/test-2-render/.*|.*/pacemaker-(3\.[^0]|[^3].*)\.rng"
	    helpers/schema-singularize.sh --clobber
	    )
	    xsltproc helpers/acls-render-convert-rng.xsl \
	      test-acls-render/pacemaker-3.0.rng > pacemaker-acls-3.0.rng
	}

	rm -f test-acls-render/pacemaker-3.0.rng
}
...
acls_render() {
	...
	find test-acls-render -name test-acls-render -o -type d -prune \
    ...
    "</catalog>"
		  ...
		  test_runner -a=base/acls-2-render -o=acls-3.0 -t=3.0 \
		              -@=$((1 << 3)) "$@"
		  _ar_ret=$?
