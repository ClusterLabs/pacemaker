 ...
 * We resolve all attributes named id-ref (which are of type IDREF). We do not
   ...
   resolution).
 * We don't set the "original" attribute for descendants in general. However, we
   treat unset "original" the same as original="0".
 -->
 ...
 * original: Boolean (1/0) indicating whether an element was part of the
             original input XML. If set, it's used as the value for the
             "original" attribute. Unset "original" attribute is treated like 0.
 -->
