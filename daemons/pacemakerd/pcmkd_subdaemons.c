...
child_liveness(pcmkd_child_t *child)
{
     ...
     */
    active_rc = pcmk__pid_active(child->pid, name);

    if ((ipc_pid != 0)
        && ((active_rc != pcmk_rc_ok)
