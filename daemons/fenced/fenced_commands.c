...
create_async_command(xmlNode *msg)
{
    ...
    op = pcmk__xpath_find_one(msg->doc, "//@" PCMK__XE_ST_DEVICE_ACTION,
                              LOG_ERR);
    if (op == NULL) {
        return NULL;
    }
...
execute_agent_action(xmlNode *msg, pcmk__action_result_t *result)
{
    ...
    dev = pcmk__xpath_find_one(msg->doc, "//" PCMK__XE_ST_DEVICE_ID, LOG_ERR);
    op = pcmk__xpath_find_one(msg->doc, "//@" PCMK__XE_ST_DEVICE_ACTION,
                              LOG_ERR);
    id = crm_element_value(dev, PCMK__XA_ST_DEVICE_ID);
    action = crm_element_value(op, PCMK__XA_ST_DEVICE_ACTION);
...
handle_query_request(pcmk__request_t *request)
{
    ...
    CRM_CHECK(request->xml != NULL, return NULL);
    ...
    dev = pcmk__xpath_find_one(request->xml->doc,
                               "//@" PCMK__XE_ST_DEVICE_ACTION, LOG_NEVER);
    if (dev != NULL) {
...
handle_relay_request(pcmk__request_t *request)
{
    ...
    CRM_CHECK(request->xml != NULL, return NULL);
    dev = pcmk__xpath_find_one(request->xml->doc, "//@" PCMK__XA_ST_TARGET,
                               LOG_TRACE);
...
handle_device_add_request(pcmk__request_t *request)
{
    ...
    dev = pcmk__xpath_find_one(request->xml->doc, "//" PCMK__XE_ST_DEVICE_ID,
                               LOG_ERR);
...
handle_device_delete_request(pcmk__request_t *request)
{
    ...
    dev = pcmk__xpath_find_one(request->xml->doc, "//" PCMK__XE_ST_DEVICE_ID,
                               LOG_ERR);
    device_id = crm_element_value(dev, PCMK_XA_ID);
    op = crm_element_value(request->xml, PCMK__XA_ST_OP);
