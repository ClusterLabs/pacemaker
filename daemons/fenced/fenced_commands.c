    ...
    if (pcmk__str_eq(device->agent, STONITH_WATCHDOG_AGENT, pcmk__str_casei)) {
        if (pcmk__strcase_any_of(cmd->action, "reboot", "off", NULL)) {
            if ((stonith_watchdog_targets == NULL) ||
                pcmk__str_in_list(stonith_watchdog_targets, stonith_our_uname)) {
...
dynamic_list_search_cb(GPid pid, int rc, const char *output, gpointer user_data)
{
    ...
    if (dev->targets) {
        ...
        if (pcmk__str_in_list(dev->targets, alias)) {
            can_fence = TRUE;
        }
    }
...
stonith_device_register(xmlNode * msg, const char **desc, gboolean from_cib)
{
    ...
    if (pcmk__str_eq(device->id, STONITH_WATCHDOG_ID, pcmk__str_none) ||
        pcmk__str_eq(device->agent, STONITH_WATCHDOG_AGENT,
                     pcmk__str_none)) do {
        ...
        } else if (!pcmk__str_eq(device->agent, STONITH_WATCHDOG_AGENT,
                                 pcmk__str_none)) {
            crm_err("Ignoring watchdog fence device with "
                    "agent '%s'!='"STONITH_WATCHDOG_AGENT"'.",
...
stonith_device_action(xmlNode * msg, char **output)
{
    ...
    if (pcmk__str_eq(id, STONITH_WATCHDOG_ID, pcmk__str_none)) {
        if (stonith_watchdog_timeout_ms <= 0) {
            ...
            xmlNode *op = get_xpath_object("//@" F_STONITH_ACTION,
                                           msg, LOG_ERR);
            const char *action = crm_element_value(op, F_STONITH_ACTION);
...
can_fence_host_with_device(stonith_device_t * dev, struct device_search_s *search)
{
    ...
    if (pcmk__str_eq(check_type, "none", pcmk__str_casei)) {
        ...
        if (pcmk__str_in_list(dev->targets, host)) {
            can = TRUE;
        } else if (g_hash_table_lookup(dev->params, PCMK_STONITH_HOST_MAP)
