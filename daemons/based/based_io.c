...
/*!
 * \internal
 * \brief Read CIB XML from \c CIB_FILE in the \c cib_root directory
 *
 * \return CIB XML parsed from \c CIB_FILE in \c cib_root , or \c NULL if the
...
compare_backup_cibs(const struct dirent **entry1, const struct dirent **entry2)
{
    ...
    time_t ctime2 = get_backup_cib_ctime((*entry1)->d_name);
 ...
 *
 * If not running in stand-alone mode, delete the current \c PCMK_XE_STATUS
 * element. Then create an empty \c PCMK_XE_STATUS child if either of the
 * following is true:
