    ...
    pcmk__if_tracing(
        {
            ...
            add_message_xml(*answer, PCMK__XE_CIB_CALLDATA, the_cib);
        },
        if (the_cib != NULL) {
            ...
            add_message_xml(*answer, PCMK__XE_CIB_CALLDATA, shallow);
            free_xml(shallow);
        }
    );
...
sync_our_cib(xmlNode * request, gboolean all)
{
    ...
    add_message_xml(replace_request, PCMK__XE_CIB_CALLDATA, the_cib);
