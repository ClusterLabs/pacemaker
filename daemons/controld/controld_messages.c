                       ...
                       gboolean prepend, const char *raised_from)
{
    ...
    last_data_id++;
    pcmk__trace("%s %s FSA input %d (%s) due to %s, %s data", raised_from,
                (prepend? "prepended" : "appended"), last_data_id,
                fsa_input2string(input), fsa_cause2string(cause),
                ((data != NULL)? "with" : "without"));
    ...
    if (data != NULL) {
        switch (cause) {
            ...
            case C_LRM_OP_CALLBACK:
                pcmk__trace("Copying %s data from %s as lrmd_event_data_t",
                            fsa_cause2string(cause), raised_from);
...
get_message(void)
{
    ...
    pcmk__trace("Processing input %d", message->id);
    return message;
}
